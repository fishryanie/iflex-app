...
const { randomDate, randomArray, randomInteger } = require('../configs/app');
...
async function update_user_groups(arrayUsers, arrayGroups){
  arrayUsers.forEach(async user => {
    await models.users.updateOne({_id: user._id}, 
      {groups: Array.from(new Set(randomArray(arrayGroups, arrayGroups.length * randomInteger(10,90) / 100))).map(item => item._id)}, 
      {new: true}
      );
  })
}
